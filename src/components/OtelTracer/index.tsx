...
export default function OtelTracer() {
  ...
  const sendPreflightRequest = useCallback(() => {
    fetch(OTEL_HTTP_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify(preflightRequestParams),
      headers: {
  ...
  const confirmReportType = useCallback(
    (isUserInterective: boolean = false) => {
  ...
  return (
    <Dialog opened={dialogOpen} size="fit">
      ...
      <SegmentedControl data={reportMechainismDescriptors} onChange={changeReportMechainism} defaultValue="custom" />
